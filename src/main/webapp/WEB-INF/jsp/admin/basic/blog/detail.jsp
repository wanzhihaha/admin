...
<section>	
	<div class="title"><h3>Blog Detail Information</h3></div>
	<article>
	...
	<table class="bd-form" summary="등록, 수정하는 영역 입니다.">
			...
			<tr>
				<th colspan="2" style="text-align: center; font-weight: bold; font-size: large; border-top: 1px solid; border-bottom: 1px solid;">Blog Information</th>
			</tr>
			<tr>
				<th scope="row">Sort1 *</th>
				<td>
					<c:out value="${detail.blogCcd1Nm }" />
				</td>
			</tr>
			<tr>
				<th scope="row">Sort2</th>
				<td>
					<c:out value="${detail.blogCcd2 }" escapeXml="false"/>
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td>
					<c:if test="${!empty detail.pcListImgOrgFileNm}">
						<img alt="<c:out value="${detail.pcListImgAlt }" escapeXml="false"/>" src="<blabProperty:value key="system.admin.path"/>/blogImgView.do?blogSeqNo=<c:out value='${detail.blogSeqNo}'/>&imgKinds=pcList" style="max-width: 100px; vertical-align: middle;">
					</c:if>		
				</td>
			</tr>
			<tr>
				...
				<td>
					<c:if test="${!empty detail.pcDetlImgOrgFileNm}">
						<img alt="<c:out value="${detail.pcDetlImgAlt }" escapeXml="false"/>" src="<blabProperty:value key="system.admin.path"/>/blogImgView.do?blogSeqNo=<c:out value='${detail.blogSeqNo}'/>&imgKinds=pcDetail" style="max-width: 100px; vertical-align: middle;">
					</c:if>		
				</td>
			</tr>
			<tr>
				...
				<td>
					<c:if test="${!empty detail.mobileListImgOrgFileNm}">
						<img alt="<c:out value="${detail.mobileListImgAlt }" escapeXml="false"/>" src="<blabProperty:value key="system.admin.path"/>/blogImgView.do?blogSeqNo=<c:out value='${detail.blogSeqNo}'/>&imgKinds=mobileList" style="max-width: 100px; vertical-align: middle;">
					</c:if>			
				</td>
			</tr>
			<tr>
				...
				<td>
					<c:if test="${!empty detail.mobileDetlImgOrgFileNm}">
						<img alt="<c:out value="${detail.mobileDetlImgAlt }" escapeXml="false"/>" src="<blabProperty:value key="system.admin.path"/>/blogImgView.do?blogSeqNo=<c:out value='${detail.blogSeqNo}'/>&imgKinds=mobileDetail" style="max-width: 100px; vertical-align: middle;">
					</c:if>		
				</td>
			</tr>
			<tr>
