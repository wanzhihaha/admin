...
<form id="listForm" action="./list.do" method="post">
	...
	<section>
		<div class="title"><h3>驾驶员信息</h3></div>
				...
				<div class="inputUI_simple">
					<table class="bd-form s-form" summary="상태, 답변 상태, 기간설정, 직접검색 영역 입니다.">
						...
						<tr>
							<th scope="row">搜索</th>
							<td>
								<input type="text" placeholder="请输入搜索名称"  id="name" name="name" value="<c:out value="${vo.name }" escapeXml="false"/>" />
							</td>
							<th scope="row">类型</th>
							<td>
								<div class="select-inner">
									<select name="type" id="type">
										...
										<option value="1" <c:if test="${1 == vo.type}">selected</c:if>>线上活动</option>
										<option value="2" <c:if test="${2 == vo.type}">selected</c:if>>线下活动</option>
									</select>
								</div>
							</td>
...
<%--							</td>--%>
			...
			<span>
				...
				<table class="bd-list inputUI_simple" summary="리스트 영역 입니다.">
					...
					<tr>
						<th scope="col">序号</th>
						<th scope="col">驾驶员类型</th>
						<th scope="col">驾驶员名称</th>
						<th scope="col">驾驶员出车次数</th>
					...
					<c:forEach var="list" items="${list }" varStatus="stat">
						<tr>
							...
							<td>
								<c:out value="${list.typeName }" escapeXml="false"/>
							</td>
							<td>
							...
							<td>
								<c:out value="${list.effectiveTime }" escapeXml="false"/>
							</td>
							<td>
								<c:out value="${list.deadTime }" escapeXml="false"/>
							</td>
                            <td>
