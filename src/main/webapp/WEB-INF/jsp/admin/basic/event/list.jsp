...
<form id="listForm" action="./list.do" method="post">
	...
	<input type="hidden" id="evtSeqNo" name="evtSeqNo" value="<c:out value="${vo.evtSeqNo }"/>"  class="inp-field widL" />
...
<section>
	...
	<table class="bd-list inputUI_simple" summary="리스트 영역 입니다.">
		<caption>번호, 정렬순서, 대표 이미지, 제목, 시작일, 종료일, 사용 유무, 조회수, 등록자, 등록일, 최종 수정자, 수정일</caption>
		<colgroup>
			<col width="3%" />
			<col width="8%" />
			<col width="8%" />
		...
		<c:forEach var="list" items="${list }" varStatus="stat">
			<tr>
				...
				<td>
					...
					<input type="hidden" name="listEvtSeqNo" value="<c:out value="${list.evtSeqNo}" />" />
				</td>
				<td>
				...
				<td style="text-align: center;">
					<c:if test="${! empty list.pcListImgOrgFileNm}">
						<a href="javascript:;"  onclick="detail(<c:out value='${list.evtSeqNo}'/>);">
							<img src="<blabProperty:value key="system.admin.path"/>/eventImgView.do?evtSeqNo=<c:out value='${list.evtSeqNo}'/>&imgKinds=pcList" width="100" alt="<c:out value="${list.pcListImgAlt }" escapeXml="false"/>" /><br />
						</a>
					</c:if>	
                </td>
				<td class="txtEll textL2">
					<a href="javascript:;"  onclick="detail('<c:out value='${list.evtSeqNo}'/>');"><c:out value='${list.titleNm}' escapeXml="false"/></a>
				</td>
				<td>
					<c:out value="${list.evtStatDtm }"/>
				</td>
				<td>
					<c:out value="${list.evtEndDtm }"/>
				</td>
				<td>
...
$(document).ready(function () {	
	...
	$.fnGetCodeSelectAjax("sGb=EVT_DIV&langCd=<c:out value="${SESSION_FORM_ADMIN.langCd}"/>", "searchType", "${vo.searchType }", "selectbox", "");
});
	//검색/페이징
	function fnSearch(){
		$("#page").val("1");
