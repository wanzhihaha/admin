...
<section>
	<div class="title"><h3>车辆信息管理</h3></div>
...
<%--		</colgroup>--%>
			...
			<td>
				<div class="select-inner">
					<select name="searchType" id="searchType">
						...
						<option value="HOT_LIST_02" <c:if test="${vo.searchType eq 'HOT_LIST_02'}">selected="selected"</c:if>>热门推荐</option>
						<option value="HOT_LIST_01" <c:if test="${vo.searchType eq 'HOT_LIST_01'}">selected="selected"</c:if>>热门产品</option>
					</select>
					<select name="useYn" id="useYn">
						<option value="">状态（全部）</option>
					</select>
				</div>
				<input type="text" placeholder="请输入标题." class="inp-field widS mglS" id="searchValue" name="searchValue" value="<c:out value="${vo.searchValue}" escapeXml="false"/>" />
			</td>
			<td>
				<div class="btn-module floatR">
					<div class="rightGroup">
						<a href="javascript:;" class="search" onclick="fnSearch();" id="btnSearch"><c:out value="搜索"/></a>
	...
	<table class="bd-list inputUI_simple" summary="리스트 영역 입니다.">
		...
		<tr>
			<th scope="col">序号</th>
			<th scope="col">排序</th>
			<th scope="col">车辆类目</th>
			<th scope="col">车辆标题</th>
			<th scope="col">车辆链接</th>
			<th scope="col">车辆状态</th>
			<th scope="col">车辆点击量</th>
			<th scope="col">创建人</th>
		...
		<c:forEach var="list" items="${list }" varStatus="stat">
			<tr>
				...
				<td>
					...
					<input type="hidden" name="listRecommendSeqNo" value="<c:out value="${list.id}" />" />
				</td>
                <td>
                    <c:out value="${list.type }"/>
                </td>
				<td class="txtEll textL2">
					<a href="javascript:;"  onclick="detail('<c:out value='${list.id}'/>');"><c:out value='${list.title}' escapeXml="false"/></a>
				</td>
                <td>
                    <c:out value="${list.link }"/>
                </td>
				<td>
					<c:out value="${list.useYnNm }"/>
				</td>
				<td>
					<c:out value="${list.srchCnt }"/>
				</td>
				<td>
	...
	<div class="btn-module mgtSM">
		...
		<div class="rightGroup"><a href="#none"  onclick="doWrite();" class="btnStyle01">新建热门</a></div>
	</div>
