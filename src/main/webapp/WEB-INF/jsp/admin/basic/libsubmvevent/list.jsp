...
<input type="hidden" id="libSubMevSeqNo" name="libSubMevSeqNo" />
<section>
	<div class="title"><h3>Library Event</h3></div>
		...
		<c:forEach var="list" items="${list}" varStatus="vs">
			<tr>
				...
				<td>
					...
					<input type="hidden" name="listMvEvtSeq" value="<c:out value="${list.libSubMevSeqNo}" />" />
				</td>
				<td>
				<c:out value="${list.evtCcdNm }" />
				</td>
				<td>
					<a href="javascript:;"  onclick="detail('<c:out value="${list.libSubMevSeqNo}" />');">
						<img alt="<c:out value="${list.pcListImgAlt }" escapeXml="false"/>" src="<blabProperty:value key="system.admin.path"/>/mvEvtImgView.do?libSubMevSeqNo=<c:out value="${ list.libSubMevSeqNo }"/>&imgKinds=pcList" style="max-width: 100px;" />
					</a>
				</td>
				<td class="txtEll textL2">
					<a href="javascript:;"  onclick="detail('<c:out value="${list.libSubMevSeqNo}" />');"><c:out value="${list.titleNm}" escapeXml="false"/></a>
				</td>
				<td>
