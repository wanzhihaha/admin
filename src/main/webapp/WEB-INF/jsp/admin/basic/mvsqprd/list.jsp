...
<input type="hidden" id="mvSqprdSeqNo" name="mvSqprdSeqNo" />
<section>
	<div class="title"><h3>Home Product</h3></div>
		...
		<c:forEach var="list" items="${list}" varStatus="vs">
			<tr>
				...
				<td>
					...
					<input type="hidden" name="listMvSqprdSeq" value="<c:out value="${list.mvSqprdSeqNo}" />" />
				</td>
				<td>
					<c:out value="${list.mvCcdNm }" />
				</td>
				<td>
					<a href="javascript:;"  onclick="detail('<c:out value="${list.mvSqprdSeqNo}" />');">
						<img alt="<c:out value="${list.pcListImgAlt }" escapeXml="false"/>" src="<blabProperty:value key="system.admin.path"/>/mvPrdImgView.do?mvSqprdSeqNo=<c:out value="${ list.mvSqprdSeqNo }"/>&imgKinds=pcList" style="max-width: 100px;" />
					</a>
				</td>
				<td class="txtEll textL2">
					<a href="javascript:;"  onclick="detail('<c:out value="${list.mvSqprdSeqNo}" />');"><c:out value="${list.sqprdNm}" escapeXml="false"/></a>
				</td>
				<td>
...
$(document).ready(function () {	
	...
	$.fnGetCodeSelectAjax("sGb=MVSQPRD_DIV&langCd=<c:out value="${SESSION_FORM_ADMIN.langCd}"/>", "searchType", "${vo.searchType}", "selectbox", "");
});
//검색 , 페이징,
