...
<form id="listForm" action="./list.do" method="post">
    ...
    <section>
        <div class="title"><h3>注册成功统计</h3></div>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="select-inner">
                                    <select name="fromSource" id="fromSource">
                                        <option value="">全部</option>
                                        <option value="3" <c:if test="${vo.fromSource eq '3' }">selected="selected"</c:if>>官网注册</option>
                                        <option value="1" <c:if test="${vo.fromSource eq '1' }">selected="selected"</c:if>>百度</option>
                                        <option value="2" <c:if test="${vo.fromSource eq '2' }">selected="selected"</c:if>>growing_io</option>
                                    </select>
                                </div>
                            </td>
                            <th scope="row">时间</th>
...
<%--                        <th scope="col">标识符</th>--%>
                        <th scope="col">M1 用户状态</th>
                        <th scope="col">M1 Organization ID</th>
                        <th scope="col">M1 用户创建时间</th>
                        <th scope="col">注册耗时(分钟)</th>
...
<%--                            </td> --%>
                            ...
                            <td>
                                <c:out value="${list.fromSource }"/>
                            </td>
<%--                            <td>--%>
<%--                                <c:out value="${list.sourceVal }"/>--%>
<%--                            </td>
                            <td>
                            ...
                            <td>
                                <c:out value="${list.organizationId }"/>
                            </td>
                            <td>
                                <c:out value="${list.member1ApprovalDate }"/>
                            </td>
                            <td>
                                <c:out value="${list.consumeTime }"/>
                            </td>
             <%--               <td>
                                <c:out value="${list.isPushBaidu }"/>
                            </td>--%>
                            <td class="ellipsis" title="${list.shareLink }">
                                <c:out value="${list.shareLink }"/>
                            </td>
                            <td class="ellipsis" title="${list.url }">
                                <span > <c:out value="${list.url }"/></span>
                            </td>
                            <td>
...
<script type="text/javascript">
    $(document).ready(function () {
        <%--$.fnGetCodeSelectAjax("sGb=REGISTER_STATISTICS_CODE&langCd=<c:out value="${SESSION_FORM_ADMIN.langCd}"/>", "fromSource", "${vo.fromSource}", "selectbox", "");--%>
    ...
    function excelDownLoad() {
        export_Excel("#listForm", "registerAppSuccess");
    }
        function fnSearch() {
            $("#page").val("1");
